...
def powerOf(b, e):
    ...
    for _ in range(e):
        result *= b
    return result
print(f'{base} to the power of {exponent} is {powerOf(base, exponent)}')
...
def even_tuple(tup):
    return tuple(int for int in tup if int%2 == 0)
print(even_tuple(int_tuple)) 
...
def list_even_numbers(integer_list):
    return [item for item in integer_list if item%2 == 0]
print(list_even_numbers(int_list))
...
def backwards(str):
    return str[::-1]
print(backwards(my_str))

""" 
7. You are given a list called mypairs. This list contains several tuples, and each tuple holds exactly 10 integers. Your task is to write two different solutions that print out each individual value inside the tuples: Manual unpacking – Do it like we did in class: unpack all 10 values into individual variables in the for-loop. Smarter unpacking – Write an alternative solution that achieves the same result, but in a more flexible or elegant way (e.g. a solution that would still work if the number of values changes in the future).
"""
print('\n7. UNPACKING TUPLES ----------------------------------------------')
mypairs = [
...
def unpackTuplesManually(pairs):
    for one, two, three, four, five, six, seven, eight, nine, ten in pairs:
        print(one, two, three, four, five, six, seven, eight, nine, ten)

def unpackTuples(pairs):
    for tup in pairs:
        for val in tup:
            print(val, end=' ')
        print('')
        
print(unpackTuplesManually(mypairs))
print(unpackTuples(mypairs))
