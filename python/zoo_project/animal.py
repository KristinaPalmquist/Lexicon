class Animal:
    ...
    def eat(self):
        if self.energy_level + self.food_energy_increase <= self.max_energy_level:
            ...
            print(f'{self.name} has eaten {self.food}.\nEnergy level is now {self.energy_level}.')
        else:
    ...
    def make_sound(self):
        if self.energy_level > self.sound_energy_decrease:
            ...
            print(f'{self.name} the {self.type} made a sound.')
        else:
            print(f'{self.name} did not have enough energy to make a sound.\nEnergy level is only {self.energy_level}.')
            
    def play(self):
        if self.energy_level > self.play_energy_decrease:
            self.change_energy_level(self.play_energy_decrease)
            print(f'{self.name} the {self.type} played with their friends.\nEnergy level is now {self.energy_level}.')
        else:
            print(f'{self.name} did not have enough energy to play.\nEnergy level is only {self.energy_level}.')